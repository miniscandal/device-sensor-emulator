...
from shared.constants.constants import MSG_CLOSE_OPERATING_PROCESS
from shared.constants.constants import STATUS_CODE_CLOSE_OPERATING_PROCESS
...
def callback_on_publish(
    ...
    print(MSG_CLOSE_OPERATING_PROCESS.format(mid=mid))
    client.disconnect()
    os._exit(1)  # type: ignore


def shut_down_device_system(
    client: CustomMqttClient,
    userdata: dict[str, str | int],
    ...
    userdata.update(
        {
            USERDATA_PROPERTY_STATUS_CODE: STATUS_CODE_CLOSE_OPERATING_PROCESS,
            USERDATA_PROPERTY_PROCEDURE_CODE: PROCEDURE_CODE_SHUT_DOWN_DEVICE_SYSTEM,
        }
    )

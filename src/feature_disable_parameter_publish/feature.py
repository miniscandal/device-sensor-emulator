...
from shared.constants.constants import MSG_DISABLE_PARAMETER_PUBLISH
from shared.constants.constants import STATUS_CODE_DISABLED_PARAMETER_PUBLISH
...
def callback_on_publish(
    ...
    print(MSG_DISABLE_PARAMETER_PUBLISH.format(mid=mid))
    client.enable_parameter_publish = False


def disable_parameter_publish(
    client: CustomMqttClient,
    userdata: dict[str, str | int],
    ...
    userdata.update(
        {
            USERDATA_PROPERTY_STATUS_CODE: STATUS_CODE_DISABLED_PARAMETER_PUBLISH,
            USERDATA_PROPERTY_PROCEDURE_CODE: PROCEDURE_CODE_DISABLE_PARAMETER_PUBLISH,
        }
    )
